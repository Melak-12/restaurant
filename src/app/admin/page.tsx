...
const Profile = () => {
          ...
          if (typeof window !== 'undefined') {
            ...
            if (tabs === 3) {
              ...
              const fetchUsers = async () => {
                try {
                  const response = await axios.get('https://backendresturant.vercel.app/api/users/getme');
  ...
  const deleteProduct=async(id:any)=>{
    ...
    try {
      const response= await axios.delete(`https://backendresturant.vercel.app/api/foods/${id}`).then(res=>{
        setIsOpen(false)
