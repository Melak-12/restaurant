...
export const fetchProductData = createAsyncThunk<Product[]>("productData/fetch", async () => {
  ...
  try {
    const response = await axios.get("https://backendresturant.vercel.app/api/foods/");
...
export const fetchMenuData = createAsyncThunk<Menu[]>("menuData/fetch", async () => {
  console.log("fetch data ");
  try {
    const response = await axios.get("https://backendresturant.vercel.app/api/menus/");
    const data = response.data;
...
export const fetchUserData = createAsyncThunk<UserData,{email:string}>("userData/fetch", async ({email}) => {
  try {
    const response = await axios.get(`https://backendresturant.vercel.app/api/users/getme?email=${email}`);
